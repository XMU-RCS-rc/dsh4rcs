...
- findings 天然是「文件 + 行号 + 说明」，映射到 dsh 的**搜索卡片**，白拿按文件折叠与点击跳转
- 规则检索按**条款**折叠，分组名带版本号（脱离版本的条款号是危险的）
- 数据经 `presentationMeta` 落到会话日志，**回放历史会话时卡片依然完整**
- **污染源排名**：把传递依赖链的「第一跳」聚合，回答"先修哪个文件能一次解锁最多下游文件"

**Tier 2 客户端面板（留接口）** —— `rcs-ui/src/panel-contract.ts` 已定数据契约与刷新策略，React 组件待实现。品牌色是中性占位，**未编造**。
