...
`npm run setup` 只读不写（除非加 `--write`），逐项告诉你还缺什么以及怎么补。**刚 clone 后不要跳过 `verify` 直接 `dsh:start`** —— 浏览器端 bundle 没构建，宿主会报缺 `lib/client.js`。
...
启动后在对话里直接问。**会话预设要选「标准模式」** —— PTC（Code）模式在 rc.6 下有已知问题，见[排错](./docs/troubleshooting.md)。
...
| `dsh-rcs-train` | 3 | 新生培训：领任务、发基线、出验收单 |
