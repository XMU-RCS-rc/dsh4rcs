...
763 个单元测试验的是逻辑，插件在真实 cordis 里的装配、每个工具的返回值对不对得上 schema 也有测试。**没人验过的是最后一层**：
...
- 应看不到：
  - `returned invalid output`（2026-09-14 修过：返回值比声明的输出 schema 多了四个字段，dsh 整次判失败）。
    修过之后还看到 `"value.reason" is not a declared property` 这类报错，是 dsh 还在跑旧构建 —— 按 [更新步骤](./install.md#更新) 重装、重启再试
...
把 `data\kb-cache` 临时改名为 `kb-cache.off`（不用重启 dsh，检索每次都现读镜像），然后输入：

> 队里有没有 CAN 总线的入门资料？

- 应看到：`rcs_kb_search` 直接报「本地镜像不存在或已损坏（……\data\kb-cache）」，并说明这不代表队里没有这份资料
- 应看不到：
  - 「本地镜像里没有检索到……」（2026-09-14 修过：镜像读不到时检索照样回「没检索到」，像是队里真没有）
  - 模型不经你同意就去调 `rcs_kb_sync`
- 做完把目录名改回 `kb-cache`
...
- 应看不到：
  - 「缺 CMake，先装 CMake 才能跑测试」
  - `returned invalid output: value is not lossless JSON`（2026-09-14 修过：探测结果里有值为 undefined 的键，dsh 整次判失败）
...
- **检查点 2：这一轮结束前弹出问答框。**
  ...
  - 详情里有带 ▶ 标记的代码片段，还有「回答会原样存在你的培训目录里……」那句，末尾是「看不懂这道题：先点「跳过此问题」，到最后一栏「追问」里问。」
  - 最后一栏是「追问（可选）」。
...
**H5** 在问答框里认真写一两句（例如「count 用 head 减 tail，绕回时加上容量」）；最后一栏「追问（可选）」点「跳过此问题」，就提交了。问答框每一项都要作答或跳过才能提交。
...
问答框弹出来后：**第 1 题点「跳过此问题」**（每一项都要作答或跳过才能提交，留空交不上去），其余照常写；在最后一栏「追问（可选）」里写一句「第 1 题问的『这次改动』指哪几行？」，然后提交。
